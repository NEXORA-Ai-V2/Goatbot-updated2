  ...
  async ask(messages) {
    try {
      return await withRetry(async () => {
        ...
        const content = res.data && (res.data.answer || res.data.result || res.data.response || res.data.reply);
        if (!content || typeof content !== "string") throw new Error("Empty response from ChatGPT API");
        return content;
      }, 1, 500);
    } catch (e) {
      ...
      try {
        ...
        const content = res.data && (res.data.answer || res.data.result || res.data.response || res.data.reply);
        if (!content || typeof content !== "string") throw new Error("Empty fallback response");
        return content;
      } catch (fallbackErr) {
  ...
  async askVision(messages, imageUrl) {
    try {
      return await withRetry(async () => {
        ...
        const content = res.data && (res.data.answer || res.data.result || res.data.response || res.data.gemini);
